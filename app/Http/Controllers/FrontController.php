...
class FrontController extends Controller
{
    ...
    public function like_dislike(Request $request){
      ...
      if($request->table=='SocialManagement')
      {
      	$appservice->create_notification(Auth::id(),'Give React on a Post',$request->id,$request->table,'global');
      	
      }elseif($request->table=='ProjectPost')
      {
      	  $dat=ProjectPost::where(['id'=>$request->id])->first();
					$p=Project::where(['id'=>$dat->project_id])->first();
					$empRel=ProjectRelation::where(['project_id'=>$dat->project_id])->get();
					foreach($empRel as $rl)
					{
					$appservice->create_notification(Auth::id(),'Give React on a '.$p->project_name.'- Post',$request->id,'ProjectPost',$rl->employee_id);
					}

					$appservice->create_notification(Auth::id(),'Give React on a '.$p->project_name.'- Post',$request->id,'ProjectPost',1);
      }elseif($request->table=='PostRelation'){
      	  $dat=PostRelation::where(['id'=>$request->id])->first();
					$g=Group::where(['id'=>$dat->group_id])->first();
					$empRel=GroupRelation::where(['group_id'=>$dat->group_id])->get();
					foreach($empRel as $rl)
					{
					$appservice->create_notification(Auth::id(),'Give React on a '.$g->group_name.'- Post',$request->id,'PostRelation',$rl->employee_id);
					}

					$appservice->create_notification(Auth::id(),'Give React on a '.$g->group_name.'- Post',$request->id,'PostRelation',1);
    ...
    {   $grp = $social = $post_data = array();
    	...
    	$Prel=ProjectRelation::where(['project_id'=>$id])->get();
    	...
    	$list=Updatefor::get();
    	return view('front.projectview',compact('list','project','Prel','social','post_data'));
    }
    ...
    public function groupDetails(Request $request,$id)
    {
    	...
    	$grprel = PostRelation::where('group_id',$id)->get();
    	$u_id=base64_decode($request->u_id);
    	if(Auth::user()->level!=NULL || Auth::user()->level!='' || Auth::user()->role_id!=12 || Auth::user()->role_id!=1)
    	{   
    	...
    	$list=Updatefor::get();
    	return view('front.groupdetails',compact('grp','grprel','post_data','list'));
    }
	...
	public function seeComment(Request $request)
	{
		...
		if(count($comment)>0){
			foreach($comment as $com){
				...
				$html.='<img src="'.$this->prbase_url.'/assets/social/images/users/user-1.jpg" alt="user" class="avatar-img"/>';
        }else{
                $html.='<img src='.$this->prbase_url.'/upload/profile/'.$user->image.' alt="user" class="avatar-img"/>';
				...
				if(count($getreply)>0){
					...
					foreach($getreply as $repl){
						...
						$html.='<img src="'.$this->prbase_url.'/assets/social/images/users/user-1.jpg" alt="user"/>';
						}else if($reuser->image && $reuser->image==''){
						$html.='<img src="'.$this->prbase_url.'/assets/social/images/users/user-1.jpg" alt="user" class="avatar-img"/>';
						}else{
						$html.='<img src='.$this->prbase_url.'/upload/profile/'.$reuser->image.' alt="user" class="avatar-img"/>';
						...
						'.$repl->comment;
						if($repl->img!=''){
						  $file2=explode(",", $repl->img);
				    	foreach($file2 as $key=>$val2){
							$html.='<a href="'.$val2.'">';
							$ext = substr($val2, strrpos($val2, '.') + 1);
							if($ext == 'jpg' || $ext == 'JPG' || $ext == 'jpeg' || $ext == 'JPEG' || $ext == 'webp' || $ext == 'png' || $ext == 'PNG'){
								$html.='<img src="'.$val2.'" style="width: 53px;display: block;">';
							}else{
								$html.='<i class="fas fa-file-alt" style="font-size: 66px;display: block;"></i>';
		...
		if ($request->hasFile('image')) {
			...
			$comm = Comment::create([
				'post_id' => $social_id,
				'user_id' => $sender_id,
				'comment' => ($request->text!='') ? $request->text : '',
				'img'=>implode(',',$file_upload),
		...
		public function projectpEdit(Request $request,$id)
		{
			...
			$appservice->create_notification(Auth::id(),'Edit a Post in -'.$p->project_name.'- Post Section',$data->id,'ProjectPost',1);
		...
		public function projectUpdate(Request $request,$id)
		{
			...
			$appservice->create_notification(Auth::id(),'Upload a Post in -'.$p->project_name.'- Post Section',$comm->id,'ProjectPost',1);
		...
		public function grouppEdit(Request $request,$id)
		{
			...
			$appservice->create_notification(Auth::id(),'Edit a post In -'.$g->group_name.'- Post Section',$data->id,'PostRelation',1);
		...
		public function postUpdate(Request $request,$id)
		{
			...
			$appservice->create_notification(Auth::id(),'Upload a post In -'.$g->group_name.'- Post Section',$comm->id,'PostRelation',1);
			return back()->with('success','Post Uploaded Successfully.');
		}
		public function giveReply(Request $request,$id)
		{
			...
			if(!$comm)
			{
				return redirect()->back()->with('error','Sorry !! Data is Deleted!!!.');
			}
      ...
      $appservice->create_notification(Auth::id(),'Post a reply on a Comment',$request->parent_id,'Comment',1);
       ...
       ///////////////////
		...
		public function showPost(Request $request,$tbl,$id)
		{
         ...
         // print_r($task);
         // exit;
			return view('front.showpost',compact('task','grp','project','social','post_data','list','tbl','name'));
