...
class AdminController extends Controller
{
    ...
    {
    try{
      $updt = User::where(['id' => Auth::id()])->first();
      $pass=$request->password;
      if ($request->hasFile('photo')) {
      $image = $request->file('photo');
      // $ext=strtolower(pathinfo($image->getClientOriginalExtension(), PATHINFO_EXTENSION));
      ...
      return redirect()->back()->with('msg','Image Not Supported!!');
      }
      }else{
      $img=$updt->image;
      }
