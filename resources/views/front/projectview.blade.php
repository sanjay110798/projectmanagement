...
.user_type_sec input[type="file"] {
	...
	<div class="middle_content">
		<div class="accordion block_accordion position-relative">
					<!-- <marquee><h5>{{$project->project_name}}</h5></marquee> -->
			<h5 style="color: #6d1715;
			font-weight: 600;
			text-shadow: -2px 4px 5px #7c7676ad;">{{$project->project_name}}</h5>

			<span class="member-list" data-toggle="modal" data-target="#memberModal"><i class="fa fa-users"></i> &nbsp;Employee List</span>
			<hr>
		...
		<div class="row">
			@foreach($Prel as $pr)
			@php 
			$user_details=App\User::where('id',$pr->employee_id)->first();
			@endphp
			<div class="col-md-12 mb-2">
			<div class="bx">
			<div class="row user_title">
				<div class="col-md-3 user_details">
					<div class="user_photo">
					@if(!$user_details)
					<img src="{{asset('assets/social/images/users/user-1.jpg')}}" alt="user"/>
					@elseif($user_details->image=='')
					<img src="{{asset('assets/social/images/users/user-1.jpg')}}" alt="user"/>
					@else
					<img src="{{asset('upload/profile/'.$user_details->image)}}" alt="user"/>
					@endif	
					</div>
				...
				<div class="col-md-9">
					...
					<h4 class="name" style="font-size: 15px;"><a href="{{route('user.post.details',['u_id'=>($user_details) ? base64_encode($user_details->id) : ''])}}">{{($user_details) ? $user_details->name : ''}}</a></h4>
					<h5 class="project" style="font-size: 16px;"><span>{{($user_details) ? $user_details->email : ''}}</span></h5>
					<br>
					</div>
				</div>
			</div>
		...
		<ul id="myTabs" class="nav nav-pills nav-justified tab_btn_sec" role="tablist" data-tabs="tabs">
			<?php
			 ...
			 if($Appservice->checkAddaccess('project.view') == true){?>
		...
		<div class="tab-content">
			...
			<div role="tabpanel" class="tab-pane fade active show" id="Videos">
				...
				<div class="block_sec" id="postupdate_show" style="display:none;">
					<form class="create_from" action="{{route('project.update',['id'=>$project->id])}}" method="post" enctype="multipart/form-data">
						@csrf
						...
						<div class="bottom_sec">
							<div class="like">
								<a href="javaScript:void(0);" class="like_dis" data-status="L" data-id="{{$v->id}}" data-table="ProjectPost">
									<div class="icon" data-toggle="tooltip" data-placement="top" title="{{$Appservice->userlist('L',$v->id,'ProjectPost')}}"><i class="fa fa-thumbs-up" aria-hidden="true"></i></div> {{$Appservice->total_likedis('L',$v->id,'ProjectPost')}}
								</a>
								<a href="javaScript:void(0);" class="like_dis" data-status="D" data-id="{{$v->id}}" data-table="ProjectPost">
									<div class="icon" data-toggle="tooltip" data-placement="top" title="{{$Appservice->userlist('D',$v->id,'ProjectPost')}}"><i class="fa fa-thumbs-down" aria-hidden="true"></i></div> {{$Appservice->total_likedis('D',$v->id,'ProjectPost')}}
								</a>
							</div>
							<div class="comment_sec">
								<a href="#0" class="btn_comment" data-table="ProjectPost" id="{{$v->id}}">
											 ...
											 <form  enctype="multipart/form-data" id="cform_{{$v->id}}">
												...
												<div class="row">
													<div class="col-md-9">
														...
														<input type="hidden" name="table" value="ProjectPost">
														<textarea class="form-control" rows="1" placeholder="Start Typing" name="text" id="comm_{{$v->id}}"></textarea>
													...
													<div class="col-md-3">
														...
														<button type="submit " class="btn btn-primary sendcomment" id="{{$v->id}}" data-table="ProjectPost" onclick="sendComments(this);">Send</button>
													</div>
												</div>
												</form>
...
.order-tracking.rejected .is-complete{
	border-color: red;
	border-width: 0px;
...
.order-tracking.rejected .is-complete:after {
	border-color: #fff;
	border-width: 0px 3px 3px 0;
	width: 7px;
	left: 11px;
