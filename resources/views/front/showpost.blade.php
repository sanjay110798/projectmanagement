...
<div class="middle_content">
		...
		font-weight: 600;
		text-shadow: -2px 4px 5px #7c7676ad;">{{($name) ? $name : ''}}</h5>
		<hr>
		</div>
		...
		<div class="tab-content">
			...
			<div role="tabpanel" class="tab-pane fade active show" id="Videos">
				...
				<div class="block_sec" id="postupdate_show" style="display:none;">
					@if($tbl=='SocialManagement')
                    <form class="create_from" action="{{route('general.post.update')}}" method="post" enctype="multipart/form-data">
					@elseif($tbl=='PostRelation')
                    <form class="create_from" action="{{route('post.update',['id'=>$task[0]->group_id])}}" method="post" enctype="multipart/form-data">
					@elseif($tbl=='ProjectPost')
                    <form class="create_from" action="{{route('project.update',['id'=>$task[0]->project_id])}}" method="post" enctype="multipart/form-data">
					@else
					<form class="create_from" action="{{route('general.post.update')}}" method="post" enctype="multipart/form-data">
					@endif
				...
				<div class="accordion block_accordion" id="block_accordion_sec">
					...
					<div class="card block_sec">
						...
						<div class="bottom_sec">
							<div class="like">
								<a href="javaScript:void(0);" class="like_dis" data-status="L" data-id="{{$v->id}}" data-table="{{$tbl}}">
									<div class="icon" data-toggle="tooltip" data-placement="top" title="{{$Appservice->userlist('L',$v->id,$tbl)}}"><i class="fa fa-thumbs-up" aria-hidden="true"></i></div> {{$Appservice->total_likedis('L',$v->id,$tbl)}}
								</a>
								<a href="javaScript:void(0);" class="like_dis" data-status="D" data-id="{{$v->id}}" data-table="{{$tbl}}">
									<div class="icon" data-toggle="tooltip" data-placement="top" title="{{$Appservice->userlist('D',$v->id,$tbl)}}"><i class="fa fa-thumbs-down" aria-hidden="true"></i></div> {{$Appservice->total_likedis('D',$v->id,$tbl)}}
								</a>
							</div>
							<div class="comment_sec">
								<a href="#0" class="btn_comment" data-table="{{$tbl}}" id="{{$v->id}}">
									{{$Appservice->total_comment($v->id,$tbl)}}<div class="icon"><i class="fa fa-comment" aria-hidden="true"></i></div>Comment
								</a>
							</div>
							<!-- ///////////// Comment/////////////////-->
											...
											<div class="comment-form align-items-center">
												...
												<div class="row">
													...
													<div class="col-md-3">
														...
														<button type="button " class="btn btn-primary sendcomment" id="{{$v->id}}" data-table="{{$tbl}}" onclick="sendComments(this);">Send</button>
													</div>
												</div>
											  </form>
